<template>
  <div>
              ...
              <li v-for="(item,index) in cartList">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCart('decre',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="editCart('incre',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="cartDel(item._id)">
                ...
                <div class="addr-new" style="cursor:pointer" @click="addPhoto(item._id,item.album)">
    ...
    <el-dialog :visible.sync="dialogAddPhotoVisible" center :title="'选择相片    '+allPhotoCount+'/25'"  width="85%">
		...
		<modal :mdShow='mdShow' @close="closeMdShow">
			<p slot="message">
				请确认是否删除！
			</p>
			<div slot="btnGroup">
	...
	methods:{
		...
		delConfirm(){
			...
			axios.post('/users/cartDel',{
				_id:this.delProductId
			}).then((res)=>{
				this.mdShow = false;
				this.checkChange = this.delProductId+'a';
		...
		checkOut(){
			...
			if(this.checkCount>0){
				this.cartList.forEach((item)=>{
					if(item.checked == 1){
						if(item.album.length<25){
							this.$alert('相片不足，请继续添加！', '错误！', {
			...
			console.log(file);
				if(file.status == 'success'){
					axios.post('/users/delPhoto',{
						cartItemId:this.cartItemId._id,
						photoId:file._id,
					}).then((resp)=>{
						let res = resp.data;
						console.log(res);
...
//  	console.log(album);
    	...
    	this.cartItemId._id = _id;
    	this.fileListCount = album.length;
